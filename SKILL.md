...
description: 使用自然语言管理生产项目、项目阶段、样机、打样、开模、试产、量产、采购、库存、发货和维修，并生成项目/生产甘特图、缺料、交期和数据补充提醒。用于 WorkBuddy 团队协同以及 SeaTable、PartDB 或本地数据操作。
---

# 生产交付协同助手（解耦版 v1.1）

通过自然语言控制生产业务数据，覆盖项目立项 → 样机/打样 → 开模 → 试产 → 量产 → 采购 → 生产执行 → 库存核对 → 发货 → 维修售后的完整周期，并支持项目甘特图、生产甘特图及四维分析。
...
11. **区分项目生命周期与生产执行阶段**：立项/打样/开模/试产/量产写入 `项目阶段`；库存核对/PCB/备料/贴片/组装/测试/出货仍使用 `生产计划.阶段`。
12. **一个项目可有多轮阶段和多个生产批次**，不得用单一“当前阶段”覆盖历史轮次。
...
### 项目阶段（新增）
- 保存立项、方案设计、工程样机、PCB打样、开模、试产、认证、量产、结项的完整历史，不替代生产计划里的执行阶段。
- 必填/提取：阶段名称、关联项目、阶段类型、状态、计划开始、计划完成、负责人。
- 阶段轮次填写 V1/V2、T0/T1、EVT/DVT/PVT、首批量产等；同一项目允许同类型多轮。
- 记录交付物、评审结论、问题与风险、下一步动作、下一步截止日。
- 工程样机、PCB打样、试产、小批量和量产阶段应关联对应生产计划；纯立项或方案阶段可为空。
- 前置阶段用于项目甘特图依赖。没有明确前置关系时不得臆造。

### 生产计划（新增）
- **填默认（不问）**：状态=`计划中`、阶段=`库存核对`、立项日期=今天。
- **必问/提取**：生产产品、数量、关联项目（列近 1 个月项目供选）、工序（列全部工序供多选，13(09测试)/14(10出货) 默认自动加）。
- **生产类型**：工程样机/PCB打样/试产/小批量/量产/返工；试产时继续填写 EVT/DVT/PVT。
- **批次信息**：填写批次号、版本号、批次目标，并关联具体 `项目阶段`。
- **排期与放行**：填写计划开始日期、计划完成日期、实际完成日期和放行状态。
