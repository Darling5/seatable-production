...
def _config_candidates() -> List[str]:
    ...
    APPDATA 在某些受限执行环境（沙箱/服务进程）里为空——微信 4.x 的配置
    恰恰装在 Roaming（%APPDATA%/Tencent/xwechat/config），漏掉它整个探测链
    就断了。所以 APPDATA 缺失时用 USERPROFILE 兜底拼 Roaming 路径
    （实测 2026-09-03：某环境 APPDATA 为空导致引擎A「未找到数据库目录」，
    而配置文件其实就在 C:/Users/<u>/AppData/Roaming/Tencent/xwechat/config）。
    """
    out = []
    bases = []
    ...
    if not os.environ.get("APPDATA"):
        ...
        if prof:
            bases.append(os.path.join(prof, "AppData", "Roaming"))
    for base in bases:
        out.extend([
