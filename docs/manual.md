...
> 这里是**深入内容**：14 张表怎么用、每个阶段该写哪张表、甘特图怎么读。
> 只想快速跑起来的话，看 [README](../README.md) 就够了。

[← 返回 README](../README.md) · [交互式使用指南（HTML）](https://darling5.github.io/seatable-production/usage-guide.html)
...
| 项目 | `状态=计划中` |
| 外壳采购记录 | `采购时间=今天`（供应商无默认，按实际填写） |
| IC采购记录 | `状态=未下单` |
| 贴片生产记录 | `状态=待送料` |
| PCBA半成品采购记录 | `状态=未下单`（供应商无默认，按实际填写） |
| 组装料采购记录 | `状态=谈判中` |
| 组装记录 | `组装厂`按实际填写 |
...
|---|---|---|---|
| 收款 | 「已打款 xx 万」 | 项目表「待收」 | 金额 ±2% 容差 → 高置信 |
| 下单 | 「下单」「返单」 | 项目表客户名 | 匹配不到 → 提示可能漏立项 |
| 客户合同 | 合同 PDF 文件 | 项目表「合同」列 | 客户名+产品词双维度，≥2 词重叠 |
| 供应商合同 | 采购合同 PDF | 5 张采购记录表 | 供应商名归一，金额差 >5% 警示 |

**只读铁律**：核对引擎绝不自动写库——高置信收款项只生成「预填意图」，人在驾驶舱或对话里
确认后才落库。台账在 `data/核对结果.csv`（本地专用，不入库）。

**驾驶舱**：老板/生产/销售页「消息↔SeaTable 核对台」——待核对表（类型/置信度徽章）、
分布统计、处置指引；高置信收款进「下一步行动建议」红字置顶。
...
```bash
python foresee.py            # 三路预测重算 → data/foresee.json + 终端风险摘要
python foresee.py --json     # 调试：直接输出 JSON
```

| 计算模块 | 回答的问题 | 方法 |
|---|---|---|
| 合同倒排 | 剩余时间够不够？哪些环节必须立刻执行？ | 已交付计划真实工期分位（当前 中位24 / p75 33 / p90 49 天）→ 各环节最晚开始日 |
...
最晚开始日徽章）、供应商画像表（类别 buffer + 风险供应商）、缺料预警表；已逾期/
高风险/必须立刻下单自动进「下一步行动建议」。

**边界（只读）**：foresee 只算不写——所有预测是建议，下单/改期仍由人决策。
