...
- 🎬 **演示视频（已上线）**：下面这段 60 秒实录使用**完全虚构的演示数据**生成，前半段是本地 `驾驶舱预览.html`（KPI / 图表 / 5 角色切换 / 夜间模式），后半段是上面的在线交互式使用指南。GitHub Pages 自动托管，直接播放：
...
> 示意图（`docs/cockpit-preview.svg`）、演示视频与交互式使用指南均为**公开模板数据**，不含任何真实业务信息与口令，可放心公开。
